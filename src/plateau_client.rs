...
pub struct Reply {
    pub kind: Kind,
    pub result: Result<Option<Profile>, &'static str>,
}
struct Shared {
    busy: bool,
    reply: Option<Reply>,
...
impl Client {
    ...
    pub fn request(&self, command: Command) -> bool {
        ...
        if state.busy || state.reply.is_some() {
            return false;
        }
        state.busy = true;
        drop(state);
        ...
        .is_err()
        {
            ...
            state.reply = Some(Reply {
                kind,
                result: Err("profile worker unavailable"),
            });
        }
        true
    }
...
pub(crate) async fn exchange_at(
    ...
    command: Command,
    url: &str,
) -> Result<Option<Profile>, &'static str> {
    if !plateau::valid_username(username) {
    ...
    let mut response = request
        .send()
        .await
        .map_err(|_| "cubesrv unavailable; press Key4 to retry")?;
    if response.status() == 404 && matches!(command, Command::Load) {
        return Ok(None);
    }
    if response.status() == 204 && matches!(command, Command::Delete) {
        return Ok(None);
    }
    if response.status() == 409 {
        return Err("profile changed on server; use Reload Saved to read it");
    }
    if !response.status().is_success() {
        return Err("profile rejected or filesystem unavailable; press Key4 to retry");
    }
    let mut bytes = alloc::vec::Vec::new();
    while let Some(chunk) = response
        .chunk()
        .await
        .map_err(|_| "profile download failed")?
    {
        if bytes.len() + chunk.len() > plateau::MAX_PROFILE_BYTES {
            return Err("profile too large");
        }
        bytes.extend_from_slice(&chunk);
    }
    let profile: Profile =
        serde_json::from_slice(&bytes).map_err(|_| "invalid profile response")?;
    if !profile.valid(username) {
        return Err("invalid profile geometry");
    }
    Ok(Some(profile))
}
