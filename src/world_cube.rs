...
impl Companion {
    ...
    pub fn visible(&self, in_world: bool) -> bool {
        in_world && self.visible
    }
    pub fn expansion(&self, now: u64) -> f32 {
        let t = now.saturating_sub(self.started) % PERIOD_MS;
        let p = if t < PERIOD_MS - 2_000 {
            0.0
        } else if t < PERIOD_MS - 1_000 {
            (t - (PERIOD_MS - 2_000)) as f32 / 1_000.0
        } else {
            (PERIOD_MS - t) as f32 / 1_000.0
        };
        p * p * (3.0 - 2.0 * p)
    }
}
...
impl Placement {
    pub fn new(width: u32, height: u32, tan_half_fov: f32, expansion: f32) -> Self {
        ...
        // Double both screen dimensions for roughly four times the visible area.
        let edge = (w.min(h) * 0.80).min(480.0);
        ...
        Self {
            center: [x * depth, y * depth, -depth],
            factor,
            spacing: grid::CUBE_COMPACT_SPACING
    ...
    pub fn pose(&self, cell: [f32; 3], basis: [[f32; 3]; 3]) -> ([f32; 3], [[f32; 3]; 3], f32) {
        ...
        (
            ...
            basis.map(orient),
            grid::CUBE_GRID_SCALE * self.factor,
        )
    }
}
...
mod tests {
    ...
    fn rising_edge_toggles_and_pulse_starts_compact_every_seven_and_half_seconds() {
        ...
        c.key(false, true, 8000);
        c.key(true, true, 8001);
        assert!(!c.visible(true));
    }
    #[test]
        ...
        for (w, h) in [
            ...
            for x in [-1., 1.] {
                for y in [-1., 1.] {
                    for z in [-1., 1.] {
                        let (p, b, s) =
                            placement.pose([x, y, z], [[1., 0., 0.], [0., 1., 0.], [0., 0., 1.]]);
                        assert!(
                            s >= 0.001,
                            "companion must remain a cube, never the HS marker path"
                        );
                        for i in [-1., 1.] {
                            for j in [-1., 1.] {
                                for k in [-1., 1.] {
                                    let v: [f32; 3] = core::array::from_fn(|a| {
                                        p[a] + s * (b[0][a] * i + b[1][a] * j + b[2][a] * k)
                                    });
                                    let px =
                                        (v[0] / (-v[2] * 0.5773503) * h as f32 + w as f32) * 0.5;
