...
struct Walker { path_active: bool }
impl Walker { fn close_path(&mut self) { self.path_active = false; } }
struct Harness {
    walker_camera:Option<Walker>,
...
fn world_picker_confirm_disable_reopen_preserves_world_and_choice() {
    ...
    let mut app=Harness {carousel:carousel::Carousel::new(ASSETS,GROUPS),asset_brush:asset_brush::Brush::new(),
        walker_camera:Some(Walker { path_active:true }),
    ...
    assert!(!app.walker_camera.as_ref().unwrap().path_active);
